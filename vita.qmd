...
paste_and <- function(words, sep = ", ", and = ", & ", and2 = and) {
  k <- length(words)
  sp <- rep(sep, k)
  sp[k] <- ""
  if (k > 1) {
...
doi2link <- function(s) {
  ifelse(
    is.na(s) | s == "",
    "",
    paste0('<a href="https://doi.org/', s, '">https://doi.org/', s, '</a> ')
  )
}
...
tibble(
  ...
  Major = c(
    "Ph.D. in Clinical Psychology [(2003)]{.smalldate}",
    "M.S. in Clinical Psychology [(1999)]{.smalldate}",
    "B.S. in Psychology [(1994)]{.smalldate}"
  ),
  Note = c(
    "Dissertation: *The Transtheoretical Model of Change in Couples*",
...
tibble::tribble(
  ...
  mutate(across(
    everything(),
    .fns = ~ str_replace_all(.x, "–", "&NoBreak;&ndash;&NoBreak;")
  )) %>%
  knitr::kable(
    align = "lll",
    escape = T,
...
#. *Non-linear Relations Among Academic Abilities Improve Clinical Practice and Empirical Interpretations* (PI) with Aidan Campagnolio (Co-PI), $12,000, Woodcock Institute Research Grant Award [(July 2025–December 2025)]{.smalldate}.
...
ggplot(ct, aes(year, cumulative_citations)) +
  ...
  geom_richtext(
    aes(label = paste0("+", cites, "<span style='color:white'>0</span>")),
    hjust = 0.5,
    vjust = 0,
    label.color = NA,
    label.padding = margin(1, 0, 0, 0, "pt"),
    label.margin = margin(0, 0, 3, 0, "pt"),
    family = "Rubik",
    size = 4.0,
    color = "gray30",
    fill = "white"
  ) +
  scale_x_continuous("Publication Year", minor_breaks = seq(2000, current_year)) +
  scale_y_continuous(
    "Cumulative Citation Count",
    expand = expansion(mult = c(0.02, 0.052))
  ) +
...
jsonlite::read_json("data/My Books.json", simplifyVector = T) %>%
  as_tibble() %>%
  mutate(
    issued = flatten(issued) %>% as.integer(),
    edition = ifelse(
      is.na(edition),
      "",
      paste0(" (", scales::ordinal(as.numeric(edition)), " ed.)")
    )
  ) %>%
  unnest(author) %>%
  mutate(
    given = str_extract_all(given, "\\b[a-zA-Z]") %>%
      map_chr(~ paste0(.x, ".", collapse = " ")),
    author = paste0(family, ", ", given)
  ) %>%
  group_by(id) %>%
  summarise(
    author = knitr::combine_words(words = author, sep = ", ", and = " & "),
    source = first(source),
    title = first(title),
  ...
  mutate(
    citation = glue::glue(
      '<div class="csl-entry">{author} ({year}). <a href="{URL}"><em>{title}</em>{edition}</a>. {publisher}.{citations}</div>'
    )
  ) %>%
  arrange(-year, author) %>%
  pull(citation) %>%
...
jsonlite::read_json("data/My Articles.json", simplifyVector = T) %>%
  as_tibble() %>%
  rename(journal = `container-title`, year = issued) %>%
  mutate(
    year = flatten(year) %>% map(`[[`, 1) %>% as.integer(),
    author = make_authors(author),
    title = na2link(title, href = URL, suffix = title_suffix(title)),
    pubid = str_remove(note, "pubid:"),
    issue = na2empty(issue, paste0("(", issue, ")")),
    page = ifelse(
      is.na(page),
      "",
      paste0(", ", str_replace(page, "-", "&ndash;"))
    ),
  ...
  mutate(
    citation = glue::glue(
      '<div class="csl-entry">{author} ({year}). {title} <em>{journal}{volume}</em>{issue}{page}. {doi2link(DOI)}{citations}</div>'
    )
  ) %>%
  arrange(-year, author) %>%
  pull(citation) %>%
...
jsonlite::read_json("data/My Chapters.json", simplifyVector = T) %>%
  rename(book = `container-title`, year = issued) %>%
  as_tibble() %>%
  mutate(
    year = flatten(year) %>% as.integer(),
    author = make_authors(author),
...
jsonlite::read_json("data/My Reports.json", simplifyVector = T) %>%
  ...
  mutate(
    ...
    publisher = paste0(
      na2empty(publisher),
      na2empty(`collection-title`),
      na2empty(`container-title`)
    ),
  ...
  mutate(
    ...
    citation = glue::glue(
      '<div class="csl-entry">{author} ({year}). <em>{title}</em> {publisher}{ifelse(publisher=="", "", ". ")}{citations}</div>'
    )
  ) %>%
  arrange(-year, author) %>%
  pull(citation) %>%
...
jsonlite::read_json("data/My Talks.json", simplifyVector = T) %>%
  as_tibble() %>%
  mutate(
    date = flatten(issued) %>% map_chr(list2date),
  ...
  mutate(
    title = paste0(title, chair, `container-title`) %>%
      str_replace_all("\\.\\.", ".")
  ) %>%
  mutate(
    citation = glue::glue(
      '<div class="csl-entry">{author} ({date}). {title} [{genre}]. {`event-title`} {na2empty(`event-place`, paste0(", ", `event-place`))}{ifelse(`event-place`=="", "", ". ")}{na2empty(URL, paste0("<a href=",URL,">",URL,"</a>"))}</div>'
    )
  ) %>%
...
*	CPSY 5519—Group Counseling [(Spring 2018–2019, 2026)]{.smalldate}
*	CPSY 5694—Introduction to Assessment [(Fall 2017–2019, 2023, 2026)]{.smalldate}
*	EDUC 5101—Critical Understanding of Social Science Research [(Fall 2018)]{.smalldate}
*	EDUC 5325—Introduction to Statistics and Research [(Fall 2017, 2021–2022, 2023–2025, Spring 2026)]{.smalldate}
*	EPSY 5529—Tests and Measurements [(Spring 2019–2020, 2022–2023)]{.smalldate}
*	EPSY 8825—Advanced Data Analysis [(Spring 2020, 2022, 2025)]{.smalldate}
*	SPSY 9687/8—Seminar in School Psychology/Psychoeducational Clinic [(Fall 2019, 2023–2024, Spring 2020–2021, 2025)]{.smalldate}
...
* Emunah Mager-Garfield [(2024)]{.smalldate}
* Octavia Blount [(2024)]{.smalldate}
* Valerie Woxholdt [(2024)]{.smalldate}
...
* I received an honorarium for a presentation I gave on 2025-11-14 to the X-BASS Institute 2025 Virtual Conference.
